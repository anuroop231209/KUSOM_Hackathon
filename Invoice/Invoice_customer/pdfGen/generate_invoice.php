...
try {
    ...
    $stmt = $conn->prepare('SELECT * FROM Invoice_customer WHERE invoice_id = :invoice_id');
    $stmt->bindValue(':invoice_id', $invoice_id, PDO::PARAM_INT);
    $stmt->execute();
    $invoice = $stmt->fetch(PDO::FETCH_ASSOC);

    // Assuming invoice_id is passed as a query parameter
    $customer_id = $invoice['company_id'];
    $product_id = $invoice['product_id'];
    // Fetching customer data
    $stmt = $conn->prepare('SELECT * FROM Company WHERE company_id = :customer_id');
